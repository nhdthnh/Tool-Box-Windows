...
# Hàm gọi dọn dẹp từ bên ngoài:
def call_cleanup_routine():
    """
    Hàm này được thiết kế để gọi từ một hàm hoặc một tệp (file) Python khác.
    Nó sẽ chạy toàn bộ ứng dụng CleanupApp với GUI.
    """
    root = tk.Tk()
    app = CleanupApp(root) # Khởi tạo đối tượng
    root.mainloop() # Chạy vòng lặp sự kiện (GUI)
